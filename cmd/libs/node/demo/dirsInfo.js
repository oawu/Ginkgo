...
var check = function(_v, closure) {
  ...
  FileSystem.exists(dirsFilePath, function(exists) {
    return !exists ?
      er(title, ['錯誤原因：' + cc('cmd/_dirs.yaml', 'w2') + ' 不存在！']) && rq('./rollback').run(_v) :
      su(title) && closure();
  });
};

var read = function(_v, closure) {
  pp((title = cc('    ➤ ', 'C') + '讀取 ' + cc('_dirs.yaml', 'w2') + ' 檔案內容') + cc('… ', 'w0'));
  FileSystem.readFile('_dirs.yaml', 'utf8', function(err, data) {
    return err ?
      er(title, ['錯誤原因：' + cc(err, 'w2')]) && rq('./rollback').run(_v) :
...
module.exports.run = function(_v, closure) {
  return check(_v, function() {
    read(_v, function(data) {
      yaml(data, function(data) {
        return closure(_v.dirs = data);
      });
    });
  });
};