...
const filterLocalFiles = closure => {
...
const s3Files     = closure => Display.line('取得 S3 上檔案') && listObjects({ Bucket: Argv.data.bucket, Prefix: Argv.data.folder }, [], closure)
const uploadFiles = closure => Display.line('上傳檔案至 S3 ') && Display.line(Argv.uploadFiles.length) && Promise.all(Argv.uploadFiles.map(file => new Promise((resolve, reject) => Argv.s3.putObject({ Bucket: Argv.data.bucket, Key: file.name, Body: FileRead(file.path), ContentType: Mime.getType(file.path) || 'text/plain', ...putS3Options }, (error, data) => error ? reject(error) : Display.line() && resolve(data))))).then(() => Display.line(true) && closure && closure()).catch(error => Display.line(false, [error.message]))
const deleteFiles = closure => Display.line('刪除 S3 的檔案') && Display.line(Argv.deleteFiles.length) && Promise.all(Argv.deleteFiles.map(file => new Promise((resolve, reject) => Argv.s3.deleteObject({ Bucket: Argv.data.bucket, Key: file.name }, (error, data) => error ? reject(error) : Display.line() && resolve(data))))).then(() => Display.line(true) && closure && closure()).catch(error => Display.line(false, [error.message]))
...
module.exports = (title, closure) => {
  ...
  Argv.localFiles.forEach(t => t.hash = Display.line() && md5File.sync(t.path))
  Display.line(true)
